    ...
    try {
      ...
      if (count($rows)) {
        ...
        foreach ($rows as $row) {
          ...
          $restaurant1 = $row->get_restaurant();
          if ($restaurant1 && (!$restaurant1->rbf_scan || $restaurant1->deleted)) {
            continue;
          }
...
//        'ACL'         => 'public-read',
  ...
  public function v2_s3_photo_get($pars = [])
  {
    //settings
    $s3_region = $this->get_setting('s3_region');
    $s3_api_key = $this->get_setting('s3_api_key');
    ...
    if (!isset($pars['restaurant_id']) || empty($pars['restaurant_id']) || empty($s3_region) || empty($s3_api_key) || empty($s3_api_secret)) {
      $this::_DEBUG ? Storage::append($this::_DEBUG_LOG_FILE_CRON, 'INVALID_CONFIG') : $this->log_failed();
      return false;
    }
    ...
    if (!$restaurant || empty($restaurant->s3_bucket_name) || empty($restaurant->s3_bucket_address)) {
      $this::_DEBUG ? Storage::append($this::_DEBUG_LOG_FILE_CRON, 'INVALID_RESTAURANT') : $this->log_failed();
      return false;
    }
    ...
    $this::_DEBUG ? Storage::append($this::_DEBUG_LOG_FILE_CRON, 'PHOTO_GET_' . date('d_M_Y_H_i_s')) : $this->log_failed();
    $this::_DEBUG ? Storage::append($this::_DEBUG_LOG_FILE_CRON, 'TIME_' . $scan_date . ' - ' . $scan_hour) : $this->log_failed();
