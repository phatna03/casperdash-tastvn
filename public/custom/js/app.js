  ...
  axios.post('/admin/restaurant/food/scan', {
    ...
    .then(response => {
      ...
      if (response.data.notify && parseInt(response.data.notify)) {
        var htmlToast = '<div class="cursor-pointer" onclick="sensor_food_scan_info(' + response.data.notify + ')">Dish found error</div>';
...
function sensor_import_foods(evt, frm) {
  evt.preventDefault();
  var form = $(frm);

  const formData = new FormData();
  formData.append('excel', form.find('input[type=file]')[0].files[0]);
  formData.append('sensor_id', form.find('input[name=sensor_id]').val());
...
function sensor_add_foods(evt, frm) {
  evt.preventDefault();
  var form = $(frm);
...
function sensor_delete_food_confirm(ele) {
  var tr = $(ele).closest('tr');
  var popup = $('#modal_delete_food_out_restaurant');
...
function sensor_delete_food(ele) {
  var popup = $(ele).closest('.modal');

  axios.post('/admin/restaurant/food/delete', {
    item: popup.find('input[name=sensor_id]').val(),
    food: popup.find('input[name=food_id]').val(),
  })
    .then(response => {
...
function sensor_get_scan_results(ele, id) {
  var parent = $(ele).closest('.dt-buttons');
  ...
  axios.post('/admin/restaurant/food/scan', {
    ...
    .then(response => {
      ...
      if (response.data.notify && parseInt(response.data.notify)) {
        var htmlToast = '<div class="cursor-pointer" onclick="sensor_food_scan_result_info(' + response.data.notify + ')">Dish found error</div>';
        message_from_toast('error', 'Notification', htmlToast);
      }
    })
    .catch(error => {
...
function sensor_food_scan_result_info(id) {
...
function sensor_food_scan_result_info_action(next = 0) {
  var popup = $('#modal_food_scan_info');
  ...
  if (arr.length) {
    ...
    //rebind
    sensor_food_scan_result_info_rebind(view_next);
  }
}

function sensor_food_scan_result_info_rebind(id) {
  var popup = $('#modal_food_scan_info');
...
function sensor_food_scan_result_select(ele) {
  var bind = $(ele);
  var form = bind.closest('form');
  var chosen = bind.val();
  if (chosen && parseInt(chosen)) {
...
function sensor_food_scan_result_update_confirm() {
  ...
  axios.post('/admin/restaurant/food/scan/get', {
    ...
    .then(response => {
      ...
      setTimeout(function () {
        popup.find('#user-update-food select').attr('onchange', 'sensor_food_scan_result_select(this)');
...
function sensor_food_scan_result_update(evt, frm) {
  evt.preventDefault();
  var form = $(frm);
  var view_current = parseInt($('body input[name=popup_view_id_itm]').val());
  ...
  axios.post('/admin/restaurant/food/scan/update', {
    ...
    .then(response => {
      ...
      message_from_toast('success', acmcfs.message_title_success, acmcfs.message_description_success_add, true);
      sensor_food_scan_result_info_rebind(view_current);
...
function sensor_food_scan_view(id) {
  axios.post('/admin/photo/view', {
    item: id,
  })
    .then(response => {
...
function sensor_food_scan_cmt(ele) {
  var parent = $(ele).closest('#lcl_wrap');
  var content = parent.find('textarea[name=note]').val();
  var object_id = parent.find('input[name=object_id]').val();
  ...
  axios.post('/admin/comment/note', {
    ...
    .then(response => {
      ...
      if ($('body .sensor_food_scan_' + object_id).length) {
        $('body .sensor_food_scan_' + object_id).attr('data-lcl-txt', content);
      }
