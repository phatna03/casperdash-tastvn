...
<tr>
  ...
  @endphp
    <td class="td_restaurant p-1 td_restaurant_{{$restaurant->id}}" data-value="{{$restaurant->id}}">
      <button type="button" onclick="kas_date_check_restaurant_data({{$restaurant->id}}, '{{$datas[$restaurant->id][$date]['date_text']}}')" class="btn btn-sm btn-secondary w-50 acm-float-right total_photos {{$dnone2}}">
        <div>
